...
class Server:
    ...
    def learn_federated_simsiam(self):
        ...
        # load weights if model paths already exists, continue training
        try:
            weights = torch.load(self.output_path, map_location=torch.device("cuda"))
            self.model.load_state_dict(weights)
        except Exception as e:
            print(e)
            print("file not found. Starting training from the beginning")
